...
1)Control camera and security module- will be used to develope a camara module which can be put on the house gate for securihy gate
...
2)Module Outside Station- ie. unit for water tank,  weather level ant to check its lid status
 its pourpose-
...
1x ESP32 : one board for each place (up to 4 lights command by ESP32) 
  5V 2 Channel Relay Module |- will be used here
to enable us to provisde the necessary amphere of current to the dimmer. : a double relay by light.
Dimmer - https://www.amazon.fr/variateur-lumière-RobotDyn-...
4 x | Generic E-DIMMER AC 50-220V 2000W SCR Triac Electric Voltage Regulator Temperature/Motor Speed Controller Light Dimmer X4|  each dimmer support a single bulb and about 4 such bulbs will be connected to to the esp 32 board to be used.
...
ESP32 
 ...
 will make use of
1 x 1 x ESP32 board- main part of the module to connect to mifi and carry out processing
